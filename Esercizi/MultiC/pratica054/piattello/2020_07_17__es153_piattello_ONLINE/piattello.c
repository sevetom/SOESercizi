...
{ 
	...
	DBGpthread_mutex_lock(&mutex,Plabel); 
	/* questo while e' una precauzione, nel caso il piattello precedenteritardi enormemente prima di cadere, il piattello attuale termina subito per non sovrapporsi nell'usare le variabili di stato del piattello */
	if( PiattelloInVolo==1 ) {
		printf("piattelo %s termina subito perche\' piattello precedente ancora in voloinizia volo\n", Plabel);
        	fflush(stdout);
		pthread_exit(NULL); 
	}
